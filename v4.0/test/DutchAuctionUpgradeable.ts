...
describe("Tests", function () {
  async function deployOneYearLockFixture() {
    ...
    const mintingFactoryForERC20 = await ethers.getContractFactory("MyToken");
    const mintingTokenForERC20 = await mintingFactoryForERC20.connect(owner).deploy(500);
    ...
    it("ERC20 Tests - Max Supply Assertion", async function(){
        ...
        describe("ERC20 and ERC721 Tests", function(){
            it("ERC20 by Owner", async function () {
                expect(await mintingTokenForERC20.mint(owner.address, 100));
            ...
            expect(await mintingTokenForERC20.mint(otherAccount.address, 400));
            ...
            it("ERC20 - Max Supply exceeded", async function () {
                await expect(mintingTokenForERC20.mint(otherAccount.address, 100)).to.be.revertedWith('Max Supply has reached');
